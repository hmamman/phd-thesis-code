...
class NeuronFair:
    def __init__(self, config, model, sensitive_param, cluster_num=4, max_global=1000, max_local=1000, max_iter=40,
                 ReLU_name="dense_4"):  # Updated ReLU_name
    ...
    def build_dnn_model(self):
        ...
        # Create the sub-model to access intermediate layer outputs
        self.sub_model = Model(inputs=self.model.inputs,
                               outputs=self.model.get_layer(self.ReLU_name).output)

        self.weights = self.get_weights()

    def get_weights(self, alpha=0.5):
        """Get neural network weights"""
        nX = copy.copy(self.X)
        senss = self.input_bounds[self.sensitive_param - 1]
        eq = np.array(nX[:, self.sensitive_param - 1] == senss[0]).astype(np.int32)
        neq = -eq + 1
        nX[:, self.sensitive_param - 1] = eq * senss[-1] + neq * senss[0]

        # Get hidden layer outputs using the sub-model
        sa = self.sub_model.predict(self.X, batch_size=256)
        nsa = self.sub_model.predict(nX, batch_size=256)
        ...
        alpha_value = sf[ti]
        return np.array(sf >= alpha_value).astype(np.int32)
    ...
    def compute_gradients(self, x, nx):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # Get hidden representations using the sub-model
            x_hidden = self.sub_model(x)
            nx_hidden = self.sub_model(nx)

            # Compute loss
            tf_weights = tf.constant(self.weights, dtype=tf.float32)
            x_loss = tf.reduce_mean(tf.square(nx_hidden * tf_weights - x_hidden * tf_weights))
            nx_loss = tf.reduce_mean(tf.square(x_hidden * tf_weights - nx_hidden * tf_weights))

        # Compute gradients
        x_grad = tape.gradient(x_loss, x)
        nx_grad = tape.gradient(nx_loss, nx)
            ...
            for iter in range(self.max_iter + 1):
                ...
                if label != n_label and (tuple(temp) not in self.global_disc_inputs) and (
                    ...
                    local_perturbation = LocalPerturbation(
                        ...
                        nx=self.nx,
                        n_value=n_value,
                        sens_param=self.sensitive_param,
...
class LocalPerturbation:
    def __init__(self, x, nx, n_value, sens_param, input_shape, parent):
    ...
    def compute_gradients(self, x, nx):
        with tf.GradientTape(persistent=True) as tape:
            ...
            # Get hidden representations using the sub-model
            x_hidden = self.parent.sub_model(x)
            nx_hidden = self.parent.sub_model(nx)

            loss = tf.reduce_mean(tf.square(nx_hidden - x_hidden))

        grad = tape.gradient(loss, x)
        ngrad = tape.gradient(loss, nx)
        del tape
        return grad, ngrad

    def __call__(self, x):
        # Similar implementation but using TF2 operations
        ...
        ind_grad, n_ind_grad = self.compute_gradients(x_tensor, nx_tensor)

        # Convert gradients to numpy for further processing
        ...
        return self.parent.clip(x + s * local_cal_grad).astype("int")
...
if __name__ == '__main__':
    ...
    nf.run(max_allowed_time=max_allowed_time)
